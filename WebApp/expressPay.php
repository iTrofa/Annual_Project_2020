...
$q->bindParam(':idService', $_GET['service']);
$q->execute();
...
switch ($_GET['package']){
    case "Hour(s)":
        ...
        $q = $DbManager->getDb()->prepare("INSERT INTO orderoptions(idOrderOption, idOrders, typeOptions, options) VALUES (?, ?, ?, ?)");
        $q->execute([
            $DbManager::v4(),
            $uuid,
            "date",
...
header('Location: stripeAPI.php?express=true');
exit;